...
VEHICLE_MAKES_LIST = [
    m['vehicle_make']
    for m in VEHICLE_MAKE_MAPPING
]

VEHICLE_MAKE_ID_MAP = {
    m['vehicle_make']: m['vehicle_make_id']
    for m in VEHICLE_MAKE_MAPPING
}
...
VEHICLE_TYPES_LIST = [
    t['vehicle_type']
    for t in VEHICLE_TYPE_MAPPING
]

VEHICLE_TYPE_ID_MAP = {
    t['vehicle_type']: t['vehicle_type_id']
    for t in VEHICLE_TYPE_MAPPING
}
...
PAYMENT_METHODS_LIST = [
    p['payment_method']
    for p in PAYMENT_METHOD_MAPPING
]

PAYMENT_METHOD_ID_MAP = {
    p['payment_method']: p['payment_method_id']
    for p in PAYMENT_METHOD_MAPPING
}
...
RIDE_STATUSES_LIST = [
    s['ride_status']
    for s in RIDE_STATUS_MAPPING
]

RIDE_STATUS_ID_MAP = {
    s['ride_status']: s['ride_status_id']
    for s in RIDE_STATUS_MAPPING
}
...
def generate_ride_confirmation(
    ...
    if ride_type:
        vehicle_type = ride_type_mapping.get(
            ride_type,
            'Standard'
        )
    else:
    ...
    # Get pricing rates from selected vehicle type
    selected_vehicle = next(
        item for item in VEHICLE_TYPE_MAPPING
        if item['vehicle_type'] == vehicle_type
    )

    base_fare = selected_vehicle['base_rate']
    per_mile_rate = selected_vehicle['per_mile']
    ...
    vehicle_make_id = VEHICLE_MAKE_ID_MAP[
        vehicle_make
    ]
    ...
    ride_confirmation = {
        ...
        'requested_payment_method': payment,
